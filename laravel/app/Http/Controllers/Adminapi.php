...
class Adminapi extends Controller
{
    ...
    {
        // Check if this is a ZenoPay mobile money payment
        if ($r->payment_gateway_type == '10') {
            return $this->handleZenoPayDeposit($r);
        }
    ...
    /**
     * Handle ZenoPay mobile money deposit
     *
     * @param Request $r
     * @return \Illuminate\Http\RedirectResponse
     */
    private function handleZenoPayDeposit(Request $r)
    {
        try {
            $user = session()->get('userlogin');
            
            // Validate required fields
            if (!$r->mobile_no || !$r->amount) {
                $message = 'Mobile number and amount are required';
                if ($r->ajax()) {
                    return response()->json(['success' => false, 'message' => $message], 400);
                }
                return redirect('/deposit?msg=' . urlencode($message));
            }

            // Validate amount (convert to TZS if needed)
            $amount = (float) $r->amount;
            $minAmount = config('zenopay.min_amount', 1000);
            $maxAmount = config('zenopay.max_amount', 100000);

            if ($amount < $minAmount) {
                $message = 'Minimum amount is ' . $minAmount . ' TSh';
                if ($r->ajax()) {
                    return response()->json(['success' => false, 'message' => $message], 400);
                }
                return redirect('/deposit?msg=' . urlencode($message));
            ...
            if ($amount > $maxAmount) {
                $message = 'Maximum amount is ' . $maxAmount . ' TSh';
                if ($r->ajax()) {
                    return response()->json(['success' => false, 'message' => $message], 400);
                }
                return redirect('/deposit?msg=' . urlencode($message));
            ...
            // Create ZenoPay service instance
            $zenoPayService = app(\App\Services\ZenoPayService::class);

            // Generate unique transaction ID
            $transactionId = uniqid('zp_', true);
            ...
            $result = $zenoPayService->createOrder($orderData);

            if ($result['success']) {
                // Save transaction record
                $trn = new Transaction;
                $trn->userid = user('id');
                $trn->platform = 'mobile_money_tanzania';
                $trn->transactionno = $transactionId;
                $trn->type = 'credit';
                $trn->amount = $amount;
                $trn->category = 'recharge';
                $trn->remark = 'ZenoPay order created - ' . $result['message'];
                $trn->status = '0'; // Pending
                $trn->save();

                // Return success response
                $message = 'Mobile money payment initiated successfully! Please complete the payment on your mobile device.';
                if ($r->ajax()) {
                ...
                return redirect('/deposit?msg=' . urlencode($message) . '&order_id=' . $transactionId);
            } else {
                // Handle ZenoPay API error
                if ($r->ajax()) {
                    return response()->json(['success' => false, 'message' => $result['message']], 400);
                }
                return redirect('/deposit?msg=' . urlencode($result['message']));
            }

        } catch (\Exception $e) {
            \Log::error('ZenoPay Deposit Error', [
                'user_id' => user('id'),
                'amount' => $r->amount,
                'mobile' => $r->mobile_no,
                'error' => $e->getMessage()
